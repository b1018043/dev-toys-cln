...
const radioButtonLabelStyle: string =
	'flex py-2 px-5 bg-gray-900 border border-gray-300 rounded-lg text-slate-300 cursor-pointer focus:outline-none hover:bg-gray-800 peer-checked:bg-gray-700 peer-checked:border-transparent';
	...
	return (
		<div>
			<div className='m-4 h-10 flex flex-row'>
				{nameFuncList.map(([name], index) => (
					<RadioButton
						...
						id={index.toString()}
						value={name}
						checked={name === nameFuncList[funcIndex][0]}
						name={name}
						label={name}
					/>
				))}
			...
			<div className='h-80 flex flex-row'>
				...
				<Editor className='basis-1/2 m-4' text={convText} editable={false} />
			</div>
		</div>
	);
};
